...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  _addAPIEventHandlers() {
    ...
    this.realtime.on('server.response.output_item.done', async (event) => {
      ...
      // Only execute tools that have a client-side handler registered via
      // `addTool`. Server-side actions/tools (e.g. robot actions) are surfaced
      // for display only and must NOT be auto-answered with a function_call_output,
      // otherwise we corrupt the conversation and trigger response loops.
      if (item?.formatted?.tool && this.tools[item.formatted.tool.name]) {
  ...
  /**
   * Returns true when an object owns a property, even if the value is null.
   * @private
   * @param {Object} object
   * @param {string} key
   * @returns {boolean}
  ...
  /**
   * Normalizes a tool definition before sending it to the realtime service.
   * @private
   * @param {ToolDefinitionType} toolDefinition
   * @returns {ToolDefinitionType}
   */
  _formatToolDefinition(toolDefinition) {
    return {
      type: 'function',
      ...toolDefinition,
    };
  ...
  /**
   * Returns client-side tool definitions registered through addTool().
   * @private
   * @returns {ToolDefinitionType[]}
   */
  _getClientToolDefinitions() {
    return Object.keys(this.tools).map((key) =>
      this._formatToolDefinition(this.tools[key].definition),
    );
  }

  /**
   * Builds the Stardust session payload from current config and client tools.
   * @private
   * @returns {StardustConfigType}
   */
  _buildStardustSession() {
    const session = JSON.parse(JSON.stringify(this.stardustConfig));
    const clientTools = this._getClientToolDefinitions();
    if (clientTools.length) {
      session.model = session.model || {};
      const configuredTools = session.model.tools || [];
      for (const tool of configuredTools) {
        if (this.tools[tool?.name]) {
          throw new Error(`Tool "${tool?.name}" has already been defined`);
        }
      }
      session.model.tools = configuredTools.concat(clientTools);
    }
    return session;
  }

  /**
   * Sends the current active session when connected.
   * @private
   * @returns {true}
   */
  _sendSessionUpdate() {
    ...
    if (this.activeConfigType === 'stardust') {
      this.realtime.send('session.update', {
        session: this._buildStardustSession(),
      });
      return true;
    }
    const session = { ...this.openaiConfig };
    session.tools = (this.openaiConfig.tools || []).concat(
      this._getClientToolDefinitions(),
    );
  ...
  updateStardustSession(sessionConfig) {
    ...
    if (this._hasOwn(sessionConfig, 'model')) {
      const { model } = sessionConfig;
      if (model === null) {
        this.stardustConfig.model = null;
      } else {
        if (!this.stardustConfig.model) this.stardustConfig.model = {};

        this.stardustConfig.model = {
          ...this.stardustConfig.model,
          ...model,
        };
      }
    ...
    if (this._hasOwn(sessionConfig, 'speech')) {
      const { speech } = sessionConfig;
      if (speech === null) {
        this.stardustConfig.speech = null;
      } else {
        if (!this.stardustConfig.speech) this.stardustConfig.speech = {};
        this.stardustConfig.speech = {
          ...this.stardustConfig.speech,
          ...speech,
        };
      }
    ...
    if (this._hasOwn(sessionConfig, 'hearing')) {
      const { hearing } = sessionConfig;
      if (hearing === null) {
        this.stardustConfig.hearing = null;
      } else {
        if (!this.stardustConfig.hearing) this.stardustConfig.hearing = {};
        this.stardustConfig.hearing = {
          ...this.stardustConfig.hearing,
          ...hearing,
        };
      }
    ...
    if (this._hasOwn(sessionConfig, 'vision')) {
      const { vision } = sessionConfig;
      if (vision === null) {
        this.stardustConfig.vision = null;
      } else {
        if (!this.stardustConfig.vision) this.stardustConfig.vision = {};
        this.stardustConfig.vision = {
          ...this.stardustConfig.vision,
          ...vision,
        };
      }
    ...
    // Update knowledge
    if (this._hasOwn(sessionConfig, 'knowledge')) {
      this.stardustConfig.knowledge = sessionConfig.knowledge;
    }

    // Update agent_id
    if (this._hasOwn(sessionConfig, 'agent_id')) {
      this.stardustConfig.agent_id = sessionConfig.agent_id;
    }

    // Handle tools
    if (sessionConfig.model?.tools) {
      this.stardustConfig.model.tools = sessionConfig.model.tools.map(
        (toolDefinition) => this._formatToolDefinition(toolDefinition),
      );
    }
