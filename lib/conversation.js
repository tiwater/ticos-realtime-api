...
export class RealtimeConversation {
  ...
  EventProcessors = {
    'conversation.item.created': (event) => {
      ...
      newItem.previous_item_id =
        event.previous_item_id ?? item.previous_item_id ?? null;
      newItem.response_id = event.response_id ?? item.response_id ?? null;
    ...
    'response.output_item.done': (event) => {
      ...
      if (!foundItem) {
        ...
        foundItem.previous_item_id = item.previous_item_id ?? null;
        foundItem.response_id = response_id ?? item.response_id ?? null;
        foundItem.formatted = {
          ...
          tool: {
            type: 'function',
            name: foundItem.name,
            call_id: foundItem.call_id,
            arguments: foundItem.arguments ?? '',
          },
        };
        this.itemLookup[foundItem.id] = foundItem;
        this.items.push(foundItem);
        this.reorderItems();
      }
      foundItem.status = item.status;
      foundItem.response_id = response_id ?? item.response_id ?? foundItem.response_id ?? null;
      if (foundItem.type === 'function_call') {
        foundItem.name = item.name ?? foundItem.name;
        foundItem.call_id = item.call_id ?? foundItem.call_id;
        foundItem.arguments = item.arguments ?? foundItem.arguments ?? '';
        foundItem.formatted.tool = foundItem.formatted.tool || {
          type: 'function',
          name: foundItem.name,
          call_id: foundItem.call_id,
          arguments: '',
        };
        foundItem.formatted.tool.name = foundItem.name;
        foundItem.formatted.tool.call_id = foundItem.call_id;
        foundItem.formatted.tool.arguments = foundItem.arguments;
      }
    ...
    'response.function_call_arguments.done': (event) => {
      const { response_id, item_id, call_id, name, arguments: args } = event;
      ...
      item.response_id = response_id ?? item.response_id ?? null;
      item.name = name ?? item.name;
      item.call_id = call_id ?? item.call_id;
      item.arguments = args ?? item.arguments ?? '';
      item.status = 'completed';
      item.formatted.tool = item.formatted.tool || {
        type: 'function',
        name: item.name,
        call_id: item.call_id,
        arguments: '',
      };
      item.formatted.tool.name = item.name;
      item.formatted.tool.call_id = item.call_id;
      item.formatted.tool.arguments = item.arguments;
      return { item, delta: { arguments: item.arguments } };
    },
  };
  ...
  /**
   * Reorders items by their protocol-level linked list.
   * Server event families can be interleaved, so item creation order is not
   * always the same as conversation order.
   */
  reorderItems() {
    const itemIds = new Set(this.items.map((item) => item.id));
    const childrenByPreviousId = {};
    for (const item of this.items) {
      const previousItemId = item.previous_item_id;
      if (previousItemId && itemIds.has(previousItemId)) {
        childrenByPreviousId[previousItemId] ||= [];
        childrenByPreviousId[previousItemId].push(item);
      }
    }
    ...
    const visit = (item) => {
      ...
      visited.add(item.id);
      ordered.push(item);
      for (const child of childrenByPreviousId[item.id] || []) {
        visit(child);
      }
    };
