...
export class RealtimeAPI extends RealtimeEventHandler {
  ...
  /**
   * Tells us whether the current URL points at the OpenAI default endpoint.
   * @private
   * @returns {boolean}
   */
  _isDefaultOpenAIUrl() {
    const current = new URL(this.url);
    const fallback = new URL(this.defaultUrl);
    return current.origin === fallback.origin && current.pathname === fallback.pathname;
  }

  /**
   * Builds the concrete WebSocket URL for the current runtime.
   * @private
   * @param {{model?: string}} [settings]
   * @returns {string}
   */
  _buildConnectionUrl({ model } = {}) {
  ...
  /**
   * Builds browser WebSocket subprotocols.
   * @private
   * @returns {string[]}
   */
  _buildBrowserProtocols() {
    const protocols = ['realtime'];
    if (this._isDefaultOpenAIUrl() && this.apiKey) {
      protocols.push(
  ...
  /**
   * Builds Node.js WebSocket options.
   * @private
  ...
  async connect(settings = {}) {
    const hasExplicitModel = Object.prototype.hasOwnProperty.call(
      settings,
      'model',
    );
    const model = hasExplicitModel
      ? settings.model
      : this._isDefaultOpenAIUrl()
    ...
    if (globalThis.WebSocket) {
      ...
      const connectionUrl = this._buildConnectionUrl({ model });
      const ws = new WebSocket(connectionUrl, this._buildBrowserProtocols());
      ...
      const connectionUrl = this._buildConnectionUrl({ model });
      const ws = new WebSocket(connectionUrl, [], this._buildNodeOptions());
      ws.on('message', (data) => {
