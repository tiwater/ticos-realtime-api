...
const ChatContainer: React.FC = () => {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      toast.error(`Disconnect error: ${err instanceof Error ? err.message : 'Unknown error'}`, {
        id: toastId,
      });
    }
  };
          ...
          <div className="p-4 space-y-4 pb-20">
            {messages.length > 0 &&
              messages.map((message, index) => (
                <Message key={message.id || index} item={message} />
              ))}
            {messages.length === 0 && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center max-w-md mx-auto p-6">
                  <AudioLines className="h-12 w-12 mx-auto mb-4 text-muted-foreground" />
                  <h3 className="text-lg font-medium mb-2">Welcome to Ticos Realtime Chat!</h3>
          ...
          <div className="pointer-events-auto flex gap-2 items-center">
            {isConnected && (
              <ChatInput
                ...
                useRealTimeRecording={!vadEnabled}
